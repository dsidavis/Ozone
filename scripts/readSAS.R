library(haven)
source("R/readSAS.R")
tars = list.files("~/Downloads", pattern = "*\\.sas7bdat$", full = TRUE)
# d = lapply(tars, haven::read_sas)
sas = lapply(tars, readSAS)
names(sas) = basename(tars)
saveRDS(sas, file = "Ozone_data_from_SAS.rds")
